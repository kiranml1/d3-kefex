...
suite.addBatch({
  "bisectLeft": {
    topic: load("arrays/bisector").expression("d3.bisectLeft"),
    ...
    "find the index of first match": function(bisect){
      var array = [1,2,2,3];
      assert.equal(bisect(array, 1), 0);
      assert.equal(bisect(array, 2), 1);
      assert.equal(bisect(array, 3), 3);
    },
    "find the insertion point od non-exact match": function(bisect){
      var array = [1, 2, 3];
  ...
  "bisectRight": {
    topic: load("arrays/bisector").expression("d3.bisectRight"),
  ...
  "bisector(comparator)": {
    topic: load("arrays/bisector").expression("d3.bisector"),
    "left": {
      topic:function(bisector){
          return bisector(function(d,x){ return _.descending(d.key, x); }).left;
      },
      "finds the index of the exact match":function(bisector){
          var array = [{key:3}, {key:2}, {key:1}];
          assert.equal(bisector(array, 3),0);
          assert.equal(bisector(array, 2),1);
          assert.equal(bisector(array, 1),2);
      },
      "finds the index of first match": function(bisect){
        var array = [{key: 3}, {key: 2}, {key: 2}, {key: 1}];
        assert.equal(bisect(array, 3), 0);
        assert.equal(bisect(array, 2), 1);
        assert.equal(bisect(array, 1), 3);
      },
      "find the insertion point of non-exact match": function(bisect){
        var array = [{key:3},{key:2},{key:1}];
